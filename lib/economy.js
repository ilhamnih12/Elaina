...
function loadEconomy() {
  if (!fs.existsSync(economyPath)) {
    fs.writeJsonSync(economyPath, { users: {} }, { spaces: 2 });
  }
...
function getUser(userId) {
  const data = loadEconomy();
  if (!data.users[userId]) return ensureUser(userId);
...
function getLeaderboard(limit = 10) {
  const data = loadEconomy();
  const arr = Object.entries(data.users).map(([id, u]) => ({ id, balance: u.balance || 0 }));
